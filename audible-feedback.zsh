LONG_TIME=20
DONT_ALERT_LONG_RUN='./configure make amake cp rsync scp wget transmission aria2c'
...
function precmd {
  ...
  if [[ $DONT_ALERT_LONG_RUN = *$PREVIOUS* ]]; then
      ;
  else
      if [ $_elapsed[-1] -ge $LONG_TIME ]; then
          notify-send "Finished Job" "[$?] $(alert_helper)"
